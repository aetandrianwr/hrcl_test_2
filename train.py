...
def train_epoch(model, train_loader, optimizer, device, loss_weights, epoch):
    model.train()
    ...
    for batch in pbar:
        ...
        pbar.set_postfix({
            'loss': f'{loss.item():.4f}',
            'l11': f'{loss_l11.item():.4f}',
            'l13': f'{loss_l13.item():.4f}',
            'l14': f'{loss_l14.item():.4f}',
            'X': f'{loss_X.item():.4f}'
        })
...
def main():
    ...
    # Model configuration - optimized for <700k parameters
    config = {
        'vocab_l11': 315,  # Max value 313 + padding + 1 extra
        'vocab_l13': 675,  # Max value 672 + padding + 1 extra
        'vocab_l14': 930,  # Max value 925 + padding + 1 extra
        'vocab_X': 1190,   # Max value 1186 + padding + 1 extra
        'd_model': 80,     # Embedding dimension
        'nhead': 4,        # Number of attention heads
        'num_layers': 2,   # Number of transformer layers (shared)
        'dropout': 0.2,
    }
    ...
    # Loss weights - prioritize exact location
    loss_weights = {
        'l11': 0.1,
        'l13': 0.2,
        'l14': 0.3,
    ...
    for epoch in range(1, num_epochs + 1):
        # Train
        train_losses = train_epoch(model, train_loader, optimizer, device, loss_weights, epoch)
        ...
        print(f"Train Loss: {train_losses['total']:.4f} "
              f"(l11: {train_losses['l11']:.4f}, l13: {train_losses['l13']:.4f}, "
              f"l14: {train_losses['l14']:.4f}, X: {train_losses['X']:.4f})")
        print(f"Val acc@1: l11={val_perf['l11']['acc@1']:.2f}%, "
              f"l13={val_perf['l13']['acc@1']:.2f}%, "
              f"l14={val_perf['l14']['acc@1']:.2f}%, "
              f"X={val_perf['X']['acc@1']:.2f}%")
        print(f"Val acc@5: l11={val_perf['l11']['acc@5']:.2f}%, "
              f"l13={val_perf['l13']['acc@5']:.2f}%, "
              f"l14={val_perf['l14']['acc@5']:.2f}%, "
              f"X={val_perf['X']['acc@5']:.2f}%")
        print(f"Val MRR: l11={val_perf['l11']['mrr']:.2f}%, "
              f"l13={val_perf['l13']['mrr']:.2f}%, "
              f"l14={val_perf['l14']['mrr']:.2f}%, "
              f"X={val_perf['X']['mrr']:.2f}%")
    ...
    if test_perf['X']['acc@1'] >= 50.0:
        ...
        print("\nStarting iterative improvement phase...")
